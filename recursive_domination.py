...
def RecursiveDomination(G: nx.Graph, ddset: set, current_vertex_index: int) -> set:
    last_vertex = list(G)[current_vertex_index - 1]

    for node in set(G.neighbors(last_vertex)) | {last_vertex}:
        neighbors = set(G.neighbors(node)) | {node}
        dominated_by = neighbors.intersection(ddset)
        if len(dominated_by) < 2:
            return ddset | {last_vertex}
    ...
    solution1 = RecursiveDomination(G, tempset, current_vertex_index)
